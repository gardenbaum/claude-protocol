...
// Runtime policy for the OMP adapter. Identical to the OpenCode policy
// module so both harnesses enforce the same rules; the duplication is
// intentional because the two harnesses ship as sibling trees (one
// resolved from .opencode/shared/, the other from .omp/shared/) and
// Bun's module cache does not share across the two.
...
function currentBranch(cwd) { return runCommand("git", ["branch", "--show-current"], cwd); }
...
function primeContext(cwd) { return runCommand("bd", ["prime"], cwd); }
